...
__memcpy(comp->output_ptr, old_ptr, old_max * sizeof (png_charp));
...
__memcpy(comp->output_ptr[comp->num_output_ptr], png_ptr->zbuf, png_ptr->zbuf_size);
         comp->num_output_ptr++;
#line 252
         png_ptr->zstream.avail_out = (uInt)png_ptr->zbuf_size;
...
#line 466
   ...
   if (!(png_ptr->flags & 0x0004))
      png_ptr->zlib_mem_level = 8;
   if (!(png_ptr->flags & 0x0008))
      png_ptr->zlib_window_bits = 15;
   if (!(png_ptr->flags & 0x0010))
      png_ptr->zlib_method = 8;
#line 494
deflateInit2_((&png_ptr->zstream),(png_ptr->zlib_level),(png_ptr->zlib_method),(png_ptr->zlib_window_bits),(png_ptr->zlib_mem_level), (png_ptr->zlib_strategy), "1.1.3", sizeof(z_stream));
   png_ptr->zstream.next_out = png_ptr->zbuf;
   png_ptr->zstream.avail_out = (uInt)png_ptr->zbuf_size;
...
#line 579
   ;
   png_write_chunk(png_ptr, (png_bytep)png_IEND, 0, (png_size_t)0);
   png_ptr->mode |= 0x10;
}
#line 587
void  
...
#line 648
   ...
   ;
   if (name == 0 || (name_len = png_check_keyword(png_ptr, name,
      &new_name)) == 0)
   {
...
#line 701
   ;
   if (spalette->name == 0 || (name_len = png_check_keyword(png_ptr,
...
png_check_keyword(png_structp png_ptr, png_charp key, png_charpp new_key)
{
   ...
   *new_key = 0;
   ...
   if (key == 0 || (key_len = __strlen(key)) == 0)
   {
...
#line 1185
   ...
   ;
   if (key == 0 || (key_len = png_check_keyword(png_ptr, key, &new_key))==0)
   {
...
#line 1226
   ...
   if (text == 0 || *text == '\0' || compression==-1)
   {
...
#line 1381
   ...
   ;
   units_len = __strlen(units) + (nparams == 0 ? 0 : 1);
   ;
...
#line 1402
   for (i = 0; i < nparams; i++)
   {
      params_len[i] = __strlen(params[i]) + (i == nparams - 1 ? 0 : 1);
      ;
...
#line 1456
   ...
   total_len = 1 + __strlen(wbuf)+1 + __strlen(hbuf);

   ;
   png_write_chunk_start(png_ptr, (png_bytep)png_sCAL, (png_uint_32)total_len);
   png_write_chunk_data(png_ptr, (png_bytep)&unit, 1);
   png_write_chunk_data(png_ptr, (png_bytep)wbuf, __strlen(wbuf)+1);
   png_write_chunk_data(png_ptr, (png_bytep)hbuf, __strlen(hbuf));
...
#line 1594
   if (png_ptr->do_filter & (0x40 | 0x20 | 0x80))
   {
      ...
      __memset(png_ptr->prev_row, 0, buf_size);
...
#line 1717
__memset(png_ptr->prev_row, 0, (png_size_t) (((png_uint_32)png_ptr->usr_channels * (png_uint_32)png_ptr->usr_bit_depth * png_ptr->width + 7) >> 3) + 1);
         return;
      }
   }
#line 1725
...
#line 1949
   ...
   mins = (~((png_uint_32)0) >> 1);
#line 1982
   if ((filter_to_do & 0x08) &&
       filter_to_do != 0x08)
   {
      png_bytep rp;
      png_uint_32 sum = 0;
