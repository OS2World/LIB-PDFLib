...
#line 59
   if (info_ptr->valid & 0x1000)
      png_write_iCCP(png_ptr, info_ptr->iccp_name, 0,
...
#line 88
   if (info_ptr->unknown_chunks_num)
   {
            ...
            up++)
       {
         ...
         if (keep != 1 &&
            up->location && (!(up->location & 0x02)) &&
            ((up->name[3] & 0x20) || keep == 3 ||
...
#line 431
   ...
   ;
#line 437
   if ((png_ptr = (png_structp)png_create_struct(0x0001)) == 0)
...
#line 463
   ...
   {
#line 479
     if (user_png_ver == 0 || user_png_ver[0] != png_libpng_ver[0] ||
...
#line 507
   png_set_filter_heuristics(png_ptr, 0,
      1, 0, 0);
#line 511
   ...
   png_size_t png_struct_size, png_size_t png_info_size)
{
   ...
   do
   {
     ...
     {
#line 538
       png_ptr->error_fn=(png_error_ptr)0;
       png_error(png_ptr,
   ...
   ;
#line 557
   __memcpy(tmp_jmp, png_ptr->jmpbuf, sizeof (jmp_buf));
#line 561
   __memset(png_ptr, 0, sizeof (png_struct));
#line 565
   __memcpy(png_ptr->jmpbuf, tmp_jmp, sizeof (jmp_buf));
#line 569
   png_ptr->zbuf_size = 8192;
   png_ptr->zbuf = (png_bytep)png_malloc(png_ptr,
      (png_uint_32)png_ptr->zbuf_size);
...
png_set_filter(png_structp png_ptr, int method, int filters)
{
   ...
   if (method == 0)
   {
      switch (filters & ((0x08 | 0x10 | 0x20 | 0x40 | 0x80) | 0x07))
      {
         ...
         case 0:  png_ptr->do_filter=0x08; break;
         case 1:   png_ptr->do_filter=0x10;  break;
         case 2:    png_ptr->do_filter=0x20;   break;
         case 3:   png_ptr->do_filter=0x40;  break;
         case 4: png_ptr->do_filter=0x80;break;
         default: png_ptr->do_filter = (png_byte)filters; break;
      }
#line 992
      if (png_ptr->row_buf != 0)
      {
         if ((png_ptr->do_filter & 0x10) && png_ptr->sub_row == 0)
         {
         ...
         if ((png_ptr->do_filter & 0x20) && png_ptr->up_row == 0)
         {
         ...
         if ((png_ptr->do_filter & 0x40) && png_ptr->avg_row == 0)
         {
            if (png_ptr->prev_row == 0)
            {
   ...
   png_doublep filter_costs)
{
   ...
   if (num_weights < 0 || filter_weights == 0 ||
      heuristic_method == 1)
   {
      num_weights = 0;
   }
...
#line 1099
      ...
      for (i = 0; i < num_weights; i++)
      {
         ...
         {
            png_ptr->inv_filter_weights[i] =
               (png_uint_16)((double)(1<<(8))*filter_weights[i]+0.5);
...
#line 1162
   for (i = 0; i < 5; i++)
   {
      if (filter_costs == 0 || filter_costs[i] < 0.0)
      {
         png_ptr->inv_filter_costs[i] =
         png_ptr->filter_costs[i] = (1<<(3));
      }
      ...
      {
         png_ptr->inv_filter_costs[i] =
            (png_uint_16)((double)(1<<(3)) / filter_costs[i] + 0.5);
...
#line 1323
   if (info_ptr->valid & 0x8000L)
       png_write_image(png_ptr, info_ptr->row_pointers);
#line 1327
   png_write_end(png_ptr, info_ptr);

   if(transforms == 0 || params == (voidp)0)
        return;
}
