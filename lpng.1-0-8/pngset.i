...
png_set_bKGD(png_structp png_ptr, png_infop info_ptr, png_color_16p background)
{
   ...
   __memcpy(&(info_ptr->background), background, sizeof(png_color_16));
...
#line 174
   ...
   if (( width > ((png_uint_32)0x7fffffffL)/rowbytes_per_pixel))
   {
      png_warning(png_ptr,
   ...
   png_charp units, png_charpp params)
{
   ...
   length = __strlen(purpose) + 1;
   ;
   info_ptr->pcal_purpose = (png_charp)png_malloc(png_ptr, length);
   __memcpy(info_ptr->pcal_purpose, purpose, (png_size_t)length);
   ...
   length = __strlen(units) + 1;
   ;
   info_ptr->pcal_units = (png_charp)png_malloc(png_ptr, length);
   __memcpy(info_ptr->pcal_units, units, (png_size_t)length);

   info_ptr->pcal_params = (png_charpp)png_malloc(png_ptr,
      (png_uint_32)((nparams + 1) * sizeof(png_charp)));
   info_ptr->pcal_params[nparams] = 0;

   for (i = 0; i < nparams; i++)
   {
      length = __strlen(params[i]) + 1;
      ;
      info_ptr->pcal_params[i] = (png_charp)png_malloc(png_ptr, length);
      __memcpy(info_ptr->pcal_params[i], params[i], (png_size_t)length);
   }
   ...
   png_color_8p sig_bit)
{
   ...
   __memcpy(&(info_ptr->sig_bit), sig_bit, sizeof (png_color_8));
             ...
             png_charp profile, png_uint_32 proflen)
{
   ...
   ;
   if (png_ptr == 0 || info_ptr == 0 || name == 0 || profile == 0)
      return;

   new_iccp_name = png_malloc(png_ptr, __strlen(name)+1);
   __strcpy(new_iccp_name, name);
   new_iccp_profile = png_malloc(png_ptr, proflen);
   __memcpy(new_iccp_profile, profile, (png_size_t)proflen);
...
;
   ...
   if (png_ptr == 0 || info_ptr == 0 || num_text == 0)
      return;
#line 475
...
__memcpy(info_ptr->text, old_text, (png_size_t)(old_max * sizeof(png_text)));
...
;
   ...
   for (i = 0; i < num_text; i++)
   {
      ...
      if (text_ptr[i].key == (png_charp)0)
          continue;

      key_len = __strlen(text_ptr[i].key);
...
#line 540
      if (text_ptr[i].text == (png_charp)0 || text_ptr[i].text[0] == '\0')
      {
         text_length = 0;
#line 548
            textp->compression = -1;
      ...
      {
         text_length = __strlen(text_ptr[i].text);
...
;
#line 562
__memcpy(textp->key, text_ptr[i].key, (png_size_t)(key_len));
      *(textp->key+key_len) = '\0';
#line 577
      {
#line 582
...
__memcpy(textp->text, text_ptr[i].text, (png_size_t)(text_length));
...
png_set_tIME(png_structp png_ptr, png_infop info_ptr, png_timep mod_time)
{
   ...
   __memcpy(&(info_ptr->mod_time), mod_time, sizeof (png_time));
...
__memcpy(&(info_ptr->trans_values), trans_values, sizeof(png_color_16));
      if (num_trans == 0)
        num_trans = 1;
   }
...
#line 660
__memcpy(np, info_ptr->splt_palettes, info_ptr->splt_palettes_num * sizeof(png_sPLT_t));
    ...
    for (i = 0; i < nentries; i++)
    {
        ...
        to->name = (png_charp)png_malloc(png_ptr,
                                        __strlen(from->name) + 1);
        __strcpy(to->name, from->name);
        to->entries = (png_sPLT_entryp)png_malloc(png_ptr,
                                 from->nentries * sizeof(png_sPLT_t));

__memcpy(to->entries, from->entries, from->nentries * sizeof(png_sPLT_t));
   ...
   png_infop info_ptr, png_unknown_chunkp unknowns, int num_unknowns)
{
    ...
    if (png_ptr == 0 || info_ptr == 0 || num_unknowns == 0)
        return;
...
#line 705
__memcpy(np, info_ptr->unknown_chunks, info_ptr->unknown_chunks_num * sizeof(png_unknown_chunk));
    ...
    for (i = 0; i < num_unknowns; i++)
    {
        ...
        __strcpy((png_charp)to->name, (png_charp)from->name);
        to->data = (png_bytep)png_malloc(png_ptr, from->size);
        __memcpy(to->data, from->data, from->size);
        to->size = from->size;
#line 720
...
{
   if(png_ptr != 0 && info_ptr != 0 && chunk >= 0 && chunk < 
   ...
   chunk_list, int num_chunks)
{
    ...
    {
      if(keep == 3 || keep == 2)
        png_ptr->flags |= 0x8000L;
      else
        png_ptr->flags &= ~0x8000L;

      if(keep == 3)
        png_ptr->flags |= 0x10000L;
      else
        png_ptr->flags &= ~0x10000L;
      return;
    }
    ...
    if(png_ptr->chunk_list != (png_bytep)0)
    {
       __memcpy(new_list, png_ptr->chunk_list, 5*old_num_chunks);
       png_free(png_ptr, png_ptr->chunk_list);
       png_ptr->chunk_list=0;
    }
    __memcpy(new_list+5*old_num_chunks, chunk_list, 5*num_chunks);
    for (p=new_list+5*old_num_chunks+4, i=0; i<num_chunks; i++, p+=5)
       *p=(png_byte)keep;
    png_ptr->num_chunk_list=old_num_chunks+num_chunks;
    png_ptr->chunk_list=new_list;

    png_ptr->free_me |= 0x0400;
